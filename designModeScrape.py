...
# Generate the setText / setProperty statements based on the type. Tricky for dropdown options - need to create an array?
index = 0
dropdowns = [{"id": "dropdownTest", "options": ["option1", "option2"]}]
for (type, element_id, string) in result:
    if type == "label" or type == "button" or type == "textarea":
